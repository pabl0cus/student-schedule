...
Типово nginx доступний лише на `127.0.0.1:3000`. У production постійні дані зберігаються в каталозі `data/`
поруч із проєктом; локальний Compose без production override використовує named volume. GPU на сервері не
потрібен. Без підключеного Windows-клієнта записи очікують у черзі — це нормальний стан.
