...
import LessonMaterials from '../../components/RecordingAttachment';
...
const ScheduleItemBase = <T extends Pair>({ scheduleMatrixCell, collapsed, children }: Props<T>) => {
  const { pair, day, date } = scheduleMatrixCell;
  const { name, tag, dates } = pair;
  ...
  return (
    <>
      ...
      <LessonMaterials pair={pair} day={day} date={date} />
    </>
  );
};
