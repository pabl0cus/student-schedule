...
const getFileExtension = (fileName: string) => {
  ...
  return extensionStart >= 0 ? fileName.slice(extensionStart).toLocaleLowerCase('uk-UA') : '';
};
...
const RecordingUploadDialog = ({
  ...
  const selectFile = (selectedFile?: File) => {
    ...
    if (selectedFile.size > MAX_RECORDING_UPLOAD_BYTES) {
      setFile(undefined);
      setFileError('Файл запису завеликий. Максимальний розмір — 500 МБ.');
      return;
    }

    if (
      !ACCEPTED_MEDIA_EXTENSIONS.includes(
        getFileExtension(selectedFile.name) as (typeof ACCEPTED_MEDIA_EXTENSIONS)[number],
      )
    ) {
      setFile(undefined);
      setFileError('Непідтримуваний формат. Оберіть аудіо або відео зі списку нижче.');
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!file) {
      setFileError('Оберіть файл запису.');
      return;
    }
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <DialogPrimitive.Title className="text-lg font-bold text-primary-font">
                Додати запис
          ...
          >
            ...
            {file ? (
              ...
              <>
                <span className="mt-3 text-sm font-bold text-primary-font">Оберіть аудіо або відео</span>
                <span className="mt-1 text-xs leading-4 text-neutral-700">
                  AAC, AVI, FLAC, M4A, M4V, MKV, MOV, MP3, MP4, OGG, WAV або WebM · до 500 МБ
                </span>
              </>
            )}
          </button>
          ...
          <div className="mt-4 rounded-xl bg-brand-00 px-3 py-2.5 text-xs leading-4 text-primary-font">
            Після завантаження запис і транскрипція будуть доступні всім, хто відкриє матеріали цієї пари.
          </div>
          ...
          <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {isSubmitting ? 'Закріплюємо розклад…' : 'Додати й транскрибувати'}
            </button>
          </div>
        </DialogPrimitive.Content>
      </DialogPrimitive.Portal>
...
export default RecordingUploadDialog;
